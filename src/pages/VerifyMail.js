...
function VerifyMail(props) {
  ...
  const sendCode = async () => {
    ...
    if (userType === 'organizer') {
      url = `https://mcqpsm.online/admin/resendverificationemail`;
    } else {
      url = `https://mcqpsm.online/user/resendverificationemail`;
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!errors && codeError.length === 0) {
      ...
      let url = null;
      if (userType === 'organizer') {
        url = `https://mcqpsm.online/admin/verifyEmail`;
      } else if (userType === 'user') {
        url = `https://mcqpsm.online/user/verifyEmail`;
      }
